...
var bundle = app.loadBundle('/guide/official/bundles/explosion');
// 发电机模型节点数据
var nodeObjData = {
...
const funcs = {
    boom: () => {
        a = a + 1;
        subObjs[0].translateX(1)
        subObjs[1].translateY(1)
        subObjs[2].translateY(-1)
        subObjs[3].translateZ(-1)
        subObjs[4].translateZ(1)
        subObjs[5].translateZ(-1)
        subObjs[6].translateX(2)
        subObjs[7].translateX(-2)
        subObjs[8].translateY(2)
        subObjs[9].translateY(-2)

    },
    reset: () => {
        subObjs[0].translateX(-1 * a)
        subObjs[1].translateY(-1 * a)
        subObjs[2].translateY(1 * a)
        subObjs[3].translateZ(1 * a)
        subObjs[4].translateZ(-1 * a)
        subObjs[5].translateZ(1 * a)
        subObjs[6].translateX(-2 * a)
        subObjs[7].translateX(2 * a)
        subObjs[8].translateY(-2 * a)
        subObjs[9].translateY(2 * a)
        a = 0
    },
}

new THING.widget.Button('展开', funcs.boom);
new THING.widget.Button('还原', funcs.reset);
new THING.widget.Button('顶牌显示',  createAllPanel);
new THING.widget.Button('隐藏',  hiddenAllPanel);
