...
// 加载场景并切换层级
const bundle = app.loadBundle('/guide/official/bundles/explosion2');
 ...
 */
function create() {
    let pipObjOriScene = app.query('["userData/addName"="newScene"]');  // 查询所有模型
    if (pipObjOriScene.length > 0) {
        for (let i = 0; i < pipObjOriScene.length; i++) {
            if (pipObjOriScene[i] != null) {
                pipeLineObj.push(pipObjOriScene[i]);
                pipPos1[pipObjOriScene[i].uuid] = pipObjOriScene[i].position;  // 原始位置赋值
                pipPosCur[pipObjOriScene[i].uuid] = pipPos1[pipObjOriScene[i].uuid];  // 当前位置赋值
            }
        }
    }
...
function scoolExpandTo() {
    ...
    app.on(THING.EventType.Wheel, function (ev) {
    ...
    for (let i = 0; i < pipeLineObj.length; i++) {
        if (pipeLineObj[i] != null) {
            ...
            if (lev && offs != 2) {
                ...
                // 各模型进行偏移，下移过程判断离地距离
                if (scoolBehi == 1 && pipPos1[obj.uuid][1] < obj.position[1]) {
                    if (obj.position[1] - pipPos1[obj.uuid][1] > heightDistance * lev) {
                        scoolruleobjOffset(obj, val, speed);
                    } else {  // 距离小于当前单次展开距离，回到原位
                        if (obj.position[1] - pipPos1[obj.uuid][1] > 0) {
                            obj.moveTo([pipPos1[obj.uuid][0], pipPos1[obj.uuid][1], pipPos1[obj.uuid][2]], {
                                time: 500,
                                orientToPath: false,
                                complete: function () {
                                    pipPosCur[obj.uuid] = obj.position;
                                    heightExpendDis = 0;
                                    $('#viewHeight .heightDistance').val(parseFloat(heightExpendDis).toFixed(1));
                                    app.camera.enableZoom = true;
                                }
...
function scoolruleobjOffset(obj, value, speed) {
    ...
    let position = [obj.position[0] + value[0], obj.position[1] + value[1], obj.position[2] + value[2]]
    obj.moveTo(position,{
        ...
        complete: function () {
            ...
            if (pipPos1[obj.uuid][1] > obj.position[1]) {
                obj.moveTo([pipPos1[obj.uuid][0], pipPos1[obj.uuid][1], pipPos1[obj.uuid][2]],{
                    time: 500,  // 移动完成需要的时间
                    orientToPath: false,
                    complete: function () {
...
function resetOri() {
    ...
    for (let i = 0; i < pipeLineObj.length; i++) {
        if (pipeLineObj[i] != null) {
            pipeLineObj[i].position = pipPos1[pipeLineObj[i].uuid];
            pipPosCur[pipeLineObj[i].uuid] = pipPos1[pipeLineObj[i].uuid];
        }
    }
}