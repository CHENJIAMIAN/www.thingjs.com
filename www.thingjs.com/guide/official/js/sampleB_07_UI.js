...
class CreateUIAnchor {
    ...
    test_create_ui() {
        ...
        car.registerComponent(THING.DOM.CSS3DComponent, 'css');

        // Create  UIAnchor
        const css = car.css;
        css.offset = [0, 3, 0];
        css.pivot = [0.5, 0];
        css.domElement = this.create_element();
     ...
     */
    create_element() {
        const srcElem = document.getElementById('board');
        const newElem = srcElem.cloneNode(true);
        newElem.style.display = "block";
        app.container.insertBefore(newElem, srcElem);
        return newElem;
    }
}
...
class CreateSlider {
    ...
    addData() {
        ...
        this.panel1.addNumberSlider(dataObj, '海拔').step(1).min(0).max(123);
...
function reset() {
    ...
    var car = app.query('car02')[0];
    car.removeComponent(THING.DOM.CSS3DComponent, 'css');
    initThingJsTip("本例程展示了平台内置的界面，点击按钮，查看效果");
}