 ...
 * @author ThingJS
 * 说明：控制摄像机
 * 功能：摄像机位置，摄像机飞行，聚焦物体，飞行到物体，摄像机跟随
 * 难度：★★★☆☆
 */

//创建app，默认绑定到 'div3d' DIV 元素
const app = new THING.App();
app.camera.addComponent(THING.EXTEND.RotateAroundComponent, 'rotateAround') //注册摄像机旋转事件
// 定义全局变量
var car;
var car02;
...
function rotate_around_obj() {
    ...
    //app.camera.position = [26.14606927122885, 7.960977531159031, 13.510069494934907]
    app.camera.rotateAround.start({
        object: car,
        time: 5000,
        yRotateAngle: 360,
        complete: () => {
            console.log('摄像机围绕物体旋转完毕！')
        },
        stop: () => {
            console.log('停止摄像机围绕物体旋转！')
        },
        update: () => {
