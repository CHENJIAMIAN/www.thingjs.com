...
var cuttingBoxOptions = {
    ...
    minRadius: 0, // 剖切面x轴一般距离
    maxRadius: 0, // 剖切面z轴一般距离
}
var clippingPlanes = [];
var clippingPlanesObject = null;
var mouseOptions = {
    mouseY: 0 // 鼠标移动的纵向距离
};
//加载场景包
const bundle = app.loadBundle('https://www.thingjs.com/uploads/wechat/5qmY5a2Q6YWx/file/测试2_0/storehouse', { ignoreTheme: true });
...
function init() {
    ...
    building = app.query('.Building')[1];
    let objs = app.query('.Floor');
    if (objs.length > 1) {
        for (let i = 1; i < objs.length; i++) {
            objs[i].renderable = true;
        }
    }
    // 设置进行剖切的建筑不可拾取
    ...
    cuttingBoxOptions.currentHeight = boundingBoxMax[1] + 0.02;
    cuttingBoxOptions.maxHeight = boundingBoxMax[1] + 0.02;
    cuttingBoxOptions.minHeight = boundingBoxMin[1] - 0.02;
    cuttingBoxOptions.minRadius = (boundingBoxMax[2] - boundingBoxMin[2]) * 0.5;
    cuttingBoxOptions.maxRadius = (boundingBoxMax[0] - boundingBoxMin[0]) * 0.5;
    // 初始剖切
    clippingPlanes = [
        { direction: [0, -1, 0], height: cuttingBoxOptions.currentHeight }, // top
    ];
    clippingPlanesObject = new THING.ClippingPlanes({
        parent: building,
        planes: clippingPlanes,
    });
    building.traverse(obj => { obj.style.clippingPlanes = clippingPlanesObject });
...
function create() {
    if (cuttingArrow == null) {
        ...
        if (objectArr.length > 0) {
            ...
            cuttingArrow = new THING.Entity({
                ...
                complete: function () {
                    registerEvent();
                    app.query('.Campus')[0].parent.add(cuttingArrow);
                }
            });
        }
        cuttingArrow.visible = false;
    ...
    let timer = setTimeout(function () {
        ...
        if (cuttingPlane == null) {
            // 创建剖切面
            let point = cuttingBoxOptions.initPos;
            let maxR = cuttingBoxOptions.minRadius;
            let minR = cuttingBoxOptions.maxRadius;
            var points1 = [[point[0] - minR, point[1], point[2] - maxR], [point[0] + minR, point[1], point[2] - maxR],
            ...
            cuttingPlane = new THING.PlaneRegion({
                type: 'Plane', // 类型
                id: 'plane', // id
                points: points1,
                position: point,
    ...
    cuttingArrow.on('mousedown', function (ev) {
        if (ev.button == 0) {
            ...
            $(document).on('mousemove', function (ev) {
                ...
                // 进行剖切
                clippingPlanes[0].height = cuttingBoxOptions.currentHeight;
               clippingPlanesObject.planes=clippingPlanes;
            })
        }
    }, '方向箭头鼠标左键按下');
...
function reset() {
    // 清除剖切
    clippingPlanes[0].height = cuttingBoxOptions.maxHeight;
    cuttingBoxOptions.currentHeight = cuttingBoxOptions.maxHeight;
    clippingPlanesObject = new THING.ClippingPlanes({
        parent: building,
        planes: clippingPlanes,
