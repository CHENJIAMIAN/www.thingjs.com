...
bundle.waitForComplete().then(() => {
    ...
    new THING.widget.Button('添加电子围栏', function () {
        ...
        // 创建电子围栏（区域）
        polygonMarker = new THING.PlaneRegion({
            points: points,
            position: [0, 0, 0],
            style: { color: '#3CF9DF', outlineColor: '#3CF9DF', opacity: 0.6 },
        });
        // 设置永远在最上层显示
    ...
    new THING.widget.Button('添加图片标注', function () {
        ...
        if (marker1) { return; }
        // 创建目标点(marker)
        marker1 = new THING.Marker({
            name: 'Marker1',
            position: coord,
            style: { image: new THING.ImageTexture('https://www.thingjs.com/guide/examples/images/navigation/user.png') },
            pivot: [0.5, 0],
        })
    })
                ...
                marker1.on('update', function () {
                    if (polygonMarker != null) {
                        ...
                        if (intoPolygonMarker) {
                            polygonMarker.style.outlineColor = '#a94442';
                            polygonMarker.style.color = '#a94442';
                            $('.warninfo1').css('display', 'block');
                            $('.warninfo2').css('display', 'none');
                            $('.warninfo3').css('display', 'none');
                        } else {
                            polygonMarker.style.outlineColor = '#3CF9DF';
                            polygonMarker.style.color = '#3CF9DF';
                            $('.warninfo1').css('display', 'none');
                            $('.warninfo2').css('display', 'none');
