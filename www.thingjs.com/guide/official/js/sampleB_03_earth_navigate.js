 ...
 */
 ...
 function nav(origin, destination, transport) {
     // 先清除导航结果
     if(thingLayer.query('.GeoLine').length > 0) {
          thingLayer.query('.GeoLine')[0].destroy();
     }
     ...
     $.ajax({
         ...
         success: function (data) {
             ...
             if (data.status === '1' || data.errcode === 0) {
                 ...
                 var road = new THING.EARTH.GeoLine({
                     name: 'road' + i,
                     coordinates: coordinates,
                     style: {
                         lineType: THING.EARTH.GeoLineType.Plane,
                         url: 'https://www.thingjs.com/uearth/uGeo/path.png',
                         width: 6,
                         speed: 0.1,
                         glowStrength: 2,
                         effect: true
 ...
 map.on('click', function (e) {
     if (selectStart) {
         ...
         var geoPoint = new THING.EARTH.GeoPoint({
             ...
             style: {
                 ...
                 // url: "https://www.thingjs.com/uearth/uGeo/start.png", // 图片 url
                 url: '/uploads/wechat/138942/file/路径导航(高德)/img/start-1.png', // 图片 url
                 size: 3, // 缩放比例
             },
         });
     ...
     if (selectEnd) {
         ...
         var geoPoint = new THING.EARTH.GeoPoint({
             ...
             style: {
                 pointType: THING.EARTH.SymbolType.Image, // 代表使用图片
                 // url: "https://www.thingjs.com/uearth/uGeo/end.png", // 图片 url
                 url: '/uploads/wechat/138942/file/路径导航(高德)/img/end-1.png', // 图片 url
                 size: 3, // 缩放比例
             },
         });
         thingLayer.add(geoPoint);
