    ...
    registerEvent() {
        ...
        app.on(THING.EventType.Click, '*', function (e) {
            if (e.button == 0) {
                ...
                app.pauseEvent(THING.EventType.Click, '*', '创建测面积平面');
            }
        }, "创建测面积平面");
    }
}
...
class DrawPolygon {
    ...
    constructor(option) {
        ...
        this.init();
        this.appClick();
        this.blueLineImage = new THING.ImageTexture('/guide/examples/images/measure/blueLine.png');
        this.redLineImage = new THING.ImageTexture('/guide/examples/images/measure/redLine.png');
        this.iflag;
    }
    ...
    init() {
        ...
        _this.template =
            `<div id="area` + _this.opts.modelNum + _this.numIndex + `" class="card-label card-label` + _this.opts.modelNum + `">
                <span class="text">
        ...
        _this.boardId = 'area' + _this.opts.modelNum + _this.numIndex;
        ...
        _this.opts.app.on('mousemove', function (e) {
            if (e.pickedPosition != undefined) {
                ...
                if (_this.pointsArr.length >= 2) {
                    ...
                    _this.rianleyDom.css('left', e.x + 10 + 'px');
                    _this.rianleyDom.css('top', e.y + 'px');
                }
            }
        }, '移动');
    ...
    createPolygon(coordinates) {
        ...
        let id = _this.opts.modelNum > 10 ? _this.opts.modelNum : '0' + _this.opts.modelNum;
        _this.regionPolygon = new THING.PlaneRegion({
            id: 'Area' + id,
        ...
        coordinates.forEach((e) => {
            areaCoordinates.push([e[0], e[2]])
        });
        _this.areaNum = Math.abs(THING.MathUtils.getArea(areaCoordinates));
        _this.template =
    ...
    createTopCard(position) {
        ...
        var box = new THING.Box(1, 1, 1, {
            name: 'box' + this.boardId,
            position: [position[0], position[1], position[2]],
            style: {
                opacity: 0,
            }
        });
        box.registerComponent(THING.DOM.CSS2DComponent, 'css');
        const css = box.css;
        css.domElement = document.getElementById(this.boardId);
        css.pivot = [0.5, 0.5];
        css.autoUpdateVisible = true;
    ...
    cardClick() {
        ...
        this.imgElem.onclick = function (ev) {
            ...
            if (this.id == _this.pointsObjArr[0].id) {
                if (_this.coordinatesArr.length < 4) {
                    _this.destroyAll();
                    _this.rianleyDom.css('display', 'none');
                    return;
                };
    ...
    initDrag(regionPoint) {
        ...
        _this.regionPoint.addComponent(THING.EXTEND.DragComponent, 'drag', {
            outlineColor: "#ff0000"
        });
        var drag = _this.regionPoint.drag;
        drag.enable = true;
        ...
        if (drag.dragStartFlag == true) {
            ...
            //_this.opts.on('mousemove',function () {
        ...
        app.on('click', (ev) => {
            if (ev.button == 2 && drag.draggingFlag == true) {
                if (_this.regionPoint) {
                    _this.regionPoint.style.color = '#ff0000';
                }
                _this.opts.app.off('mousemove', null, '拖动节点');
        ...
        // 拖拽结束
        // if (drag.draggingFlag == true) {
        //     _this.regionPoint.on('dragend', function (ev) {
        //     //_this.opts.app.on('mouseup', function (ev) {
        //         if (_this.regionPoint) {
        //             _this.regionPoint.style.color = '#ff0000';
        //         }
        //         _this.opts.app.off('mousemove', null, '拖动节点');
    ...
    appOff() {
        ...
        this.opts.app.off('mousemove', null, '拖动节点');
    }
    ...
    destroyAll() {
        ...
        if (this.opts.app.query('points' + this.opts.modelNum)) {
     ...
     */
        ...
        setTimeout(() => {
            ...
            // p.on(THING.EventType.MouseUP, function (e) {
            //     if (e.object) {
            //         $('#polygonRegion' +  this.opts.modelNum).children('span').text('占地面积: ' + this.areaNum.toFixed(2) + ' 平方米')
            //     }
            // })
            _this.opts.app.resumeEvent(THING.EventType.Click, '*', '创建测面积平面');
        }, 500)
    }
     ...
     */
    dashLine(points) {
        let dashLine = new THING.PolygonLine({
    ...
    panelText() {
        ...
        tr += `平方米</td></tr>`;
        this.table.append(tr);
        if ($('#polygonRegion' + this.opts.modelNum)) {
            $('#polygonRegion' + this.opts.modelNum).children('span').text('占地面积: ' + this.areaNum.toFixed(2) + ' 平方米')
        }
    }
