...
function setOpacity() {
    ...
    car.style.opacity = (opacity > 0.8) ? 0.5 : 1.0;
...
function setColor() {
    ...
    car.style.color = (color != '#FF0000') ? '#ff0000' : '#00ff00';
...
function buttonCheck() {
    ...
    if (posInfo[9].value == "开启编辑") {
        ...
        car.trigger('singleclick');  // 出发点击事件来给物体添加控制轴
        car.addControl(new THING.AxisTransformControl(car), 'axisControl');  // 添加控制轴
        posInfo[9].value = "关闭编辑";
    } else {
        car.removeControl('axisControl');  // 移除控制轴
